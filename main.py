...
from app.node_functions.chains.tools.models.constants import LANGCHAIN_TRACING,LANGCHAIN_PROJECT,LAST
    ...
    for msg in st.session_state.messages:
        with st.chat_message(msg["role"]):
            ...
            if msg.get("underwriting_audit"):
                pprint("QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ")
            ...
            if msg.get("product_response_answer"):
                st.markdown("product_response_answer : " + msg["product_response_answer"])
    ...
    with st.chat_message("assistant"):
        try:
            with st.spinner("Retrieving docs and generating answer…"):
                ...
                if ga:
                    is_grounded = getattr(ga, "is_grounded", None) or ga.get("is_grounded") if isinstance(ga,
                                                                                                          dict) else None
                    grounding_answer = getattr(ga, "grounding_agent_answer", None) or ga.get(
                        "grounding_agent_answer") if isinstance(ga, dict) else None
                ...
                if pr:
                    pr_answer = getattr(pr, "answer", None) or pr.get("answer") if isinstance(pr, dict) else None
                    pr_audit = getattr(pr, "audit", None) or pr.get("audit") if isinstance(pr, dict) else None
                ...
                if ur:
                    ur_answer = getattr(ur, "answer", None) or ur.get("answer") if isinstance(ur, dict) else None
                    ur_audit = getattr(ur, "audit", None) or ur.get("audit") if isinstance(ur, dict) else None
