...
from app.node_functions.node_functions import (
    judge_reason,
    uw_agent_reason,
    MedicareMessageGraph,
    categorize_agent_reason,
    product_agent_reason,
    product_grounding_reason,
)
from app.node_functions.mcp_tools import uw_tool_node, product_tool_node

# from app.node_functions.node_functions import product_tool_node
...
def after_categorize_router(state: MedicareMessageGraph):
    ...
    if category == "PRODUCT":
        ...
        return END
medicare_graph.add_conditional_edges(
    CATEGORIZE_AGENT_REASON,
...
def after_judge_router(state: MedicareMessageGraph):
    if int(state["judge_evaluation_score"]) < 3:
        return END
    else:
        return PRODUCT_AGENT_REASON
medicare_graph.add_conditional_edges(
    JUDGE_REASON,
    after_judge_router,
    {END: END, PRODUCT_AGENT_REASON: PRODUCT_AGENT_REASON},
)
...
def after_uw_router(state: MedicareMessageGraph) -> str:
    ...
    return UW_TOOL_NODE
medicare_graph.add_conditional_edges(
    UW_AGENT_REASON, after_uw_router, {END: END, UW_TOOL_NODE: UW_TOOL_NODE}
)
...
def run_graph(InvokeRequest: InvokeRequest) -> MedicareMessageGraph:
    result = medicare_graph.invoke(
        {
            "messages": [HumanMessage(content=InvokeRequest.query)],
            "uw_agent_messages": [HumanMessage(content=InvokeRequest.query)],
        }
    )
    return result
...
if __name__ == "__main__":
    ...
    uw_query_1 = "Evaluate the underwriting application. Its for state of Georgia, the start date of the medicare insurance is from 1st February 2026. Applicant is 65 years old, no prior coverage and no major health history"
    ...
    InvokeRequest = InvokeRequest(query=product_query_2)
