...
class GroundingAgentResponse(BaseModel):
    is_grounded: Optional[bool] = Field(
        default="", description="Whether the answer is grounded in retrieved evidence."
    )
    grounding_agent_answer: Optional[str] = Field(
        default="", description="The final natural-language answer"
    )


class JudgeAgentResponse(BaseModel):
    evaluation: Optional[str] = Field(
        default="", description="rationale for the rating, as a text"
    )
    score: Optional[int] = Field(
        default="", description="rating, as a number between 1 and 5"
    )
